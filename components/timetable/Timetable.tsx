...
import { Meeting, partitionMeetingsByDay } from "./Meeting"
  ...
  for (
    ...
    for (const day of days) {
      ...
      for (const group of groupsByDay!.get(day) as any) {
        ...
        const rowspan = Math.ceil((groupEndTime - groupStartTime) / resolution)
        ...
        if (group.meetings.length === 1) {
          ...
          cells.push(
            ...
            >
              <MeetingTime
                ...
                dark={dark}
                highlight={
                  hoverCourseKey === meeting.courseKey && emphasizeOnHover
                }
          ...
          group.meetings.forEach((meeting: Meeting, index: number) => {
            const height =
            ...
            items.push(
              <MeetingTime
                ...
                style={{
                  ...
                  top: `calc(${
                    ((meeting.startTime - groupStartTime) /
                      (groupEndTime - groupStartTime)) *
                    100
                  }% + 0.1rem)`,
                  backgroundColor:
      ...
      if (!isOccupied) {
        cells.push(<MeetingTimeCell key={day} days={days.length} dark={dark} />)
      }
    }
    tableRows.push(
      <StyledTr
